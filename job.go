...
// RenameJob 重命名job
func (j *JenkinsSdk) RenameJob(job *JenkinsJob, newName string) error {
	
	// 创建请求 URL
	api := fmt.Sprintf("%s%v/doRename?newName=%s", j.Url, job.GetFullPath(), newName)
	//fmt.Println(api)
	...
	_, err = j.sendHttp(req)
	if err != nil {
		return errors.New("rename job error: " + err.Error())
	}
